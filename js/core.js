    ...
    const PREGUNTAS_ESPOSO = ['nombre_esposo', 'fecha_esposo', 'lugar_esposo'];
    const PASOS_PRE_DS160 = 5;
    const PASOS_POST_DS160 = 8; 
    const TOTAL_PASOS = PASOS_PRE_DS160 + cuestionarioDS160.length + PASOS_POST_DS160; 
    ...
    function renderScreen(pasoForzado = null) {
        ...
        switch(step) {
            case 0: html = `<h3>¡Bienvenido!</h3><p>Captura tu información paso a paso.</p><button onclick="guardarDato('pais', 'USA', 1)">Iniciar Trámite USA 🇺🇸</button>`; break;
            ...
            case 5:
                ...
                if (idx >= cuestionarioDS160.length) { avanzarPaso(6); return; }
                ...
                html += `<div class="tip-box"><strong>💡 Consejo:</strong><br>${q.tip}</div>
                         <button onclick="guardarRespuestaDS160('${q.id}', '${q.tipo}')">Guardar y Siguiente</button>
    ...
    async function buscarCP() {
        ...
        msg.innerText = "Buscando..."; document.getElementById('direccion_detalles').style.display = 'block';
        try {
            let res = await fetch(`https://api.zippopotam.us/mx/${cp}`);
            if(!res.ok) throw new Error();
            let data = await res.json();
            sel.innerHTML = ''; data.places.forEach(p => sel.innerHTML += `<option value="${p['place name']}, ${data.places[0].state}">${p['place name']}</option>`);
            msg.innerText = `Ubicación: ${data.places[0].state} ✅`; msg.style.color="green";
        } catch(e) { msg.innerText="CP no encontrado."; msg.style.color="red"; sel.innerHTML='<option value="">Manual</option>'; }
    }
    ...
    function guardarRespuestaDS160(id, tipo) {
        ...
        localStorage.setItem('datosVisado', JSON.stringify(appData)); renderScreen(5); 
    }
